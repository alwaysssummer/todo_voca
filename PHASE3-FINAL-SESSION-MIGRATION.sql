-- ===================================================================
-- Phase 3: 완전한 "회차(Session)" 전환 - 최종 마이그레이션
-- ===================================================================
-- 목적: "Day" 개념을 완전히 제거하고 "회차(Session)" 기반으로 전환
-- ===================================================================

-- ========================================
-- 1. get_next_word 함수: 날짜 필터링 제거
-- ========================================
...
BEGIN
    ...
    WHERE w.wordlist_id = v_wordlist_id
      ...
      -- ⭐ 학습 상태 필터링 (날짜 조건 완전 제거!)
      AND (
        swp.status IS NULL 
        OR swp.status = 'not_started'
        OR swp.status = 'skipped'  -- ⭐ 단순화: Skip이면 즉시 재등장
      )
    ORDER BY 
      -- ⭐ Skip 단어 우선 (skip_count가 높을수록 우선)
      CASE WHEN swp.status = 'skipped' THEN 0 ELSE 1 END,
      swp.skip_count DESC NULLS LAST,
...
'학생의 다음 학습 단어를 가져옵니다. Skip 단어는 즉시 재등장하며, skip_count가 높을수록 우선 순위가 높습니다.';
...
-- 2-2. completed_wordlists 테이블에 session_number 존재 확인
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'completed_wordlists'
  AND column_name IN ('session_number', 'day_number');

-- 2-3. student_word_progress에 session_number 존재 확인
SELECT column_name, data_type
FROM information_schema.columns
WHERE table_name = 'student_word_progress'
  AND column_name = 'session_number';

SELECT '✅ Phase 3 마이그레이션 완료!' as status;
SELECT '⭐ 이제 "Day" 개념이 완전히 제거되고 "회차(Session)" 기반으로 전환되었습니다!' as message;
